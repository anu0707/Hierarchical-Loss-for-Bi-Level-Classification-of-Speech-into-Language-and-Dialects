...
le=len('/media/data/CygNet_DL2/ananya/LARGE/train/kan/')
le1=len('/media/data/CygNet_DL2/ananya/LARGE/train/')
#print(le)
def lstm_data(f):
    X = torch.load(f)   
...
files_list = []
main_folder_path = '/media/data/CygNet_DL2/ananya/LARGE/train/'
...
l = len(files_list)
txtfl = open('/media/data/CygNet_DL2/ananya/LARGE/retrain/train.txt', 'w') # txt file to write training loss and accuracies afte every epoch.
...
for e in range(n_epoch): # repeat for n_epochc
    i = 0
    cost = 0.0
    ...
    for fn in files_list:                          
        ...
        print("x-vec-mfcc.py: Epoch = ",e+1,"  completed files  "+str(i)+"/"+str(l)+" Loss= %.7f"%(cost/i))
    ...
    txtfl.write('\n')
    txtfl.write("lloss"+str(lloss))
    txtfl.write('\n')
    txtfl.write("dloss"+str(dloss))    
    txtfl.write('\n')
